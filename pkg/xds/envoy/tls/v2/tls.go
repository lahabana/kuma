...
func CreateDownstreamTlsContext(ctx xds_context.Context, metadata *core_xds.DataplaneMetadata, isPublicIngress bool) (*envoy_auth.DownstreamTlsContext, error) {
	var err error
	var commonTlsContext *envoy_auth.CommonTlsContext
	if isPublicIngress {
		if envoy_tls.GatewaySecretPath() == "" {
			return nil, nil
		}
		validationSANMatcher := MeshSpiffeIDPrefixMatcher(envoy_tls.GatewayMesh)
		// Default can talk to any other meshes
		commonTlsContext, err = createCommonTlsContext(ctx, metadata, envoy_tls.GatewayCaResource, envoy_tls.GatewayCertResource, validationSANMatcher)
		...
		if envoy_tls.GatewaySecretPath() != "" && ctx.Mesh.Resource.Meta.GetName() != "default" {
			validationSANMatchers = append(validationSANMatchers, MeshSpiffeIDPrefixMatcher("default"))
		}
		// Default can talk to any other meshes
		commonTlsContext, err = createCommonTlsContext(ctx, metadata, envoy_tls.MeshCaResource, envoy_tls.IdentityCertResource, validationSANMatchers...)
	} else {
		return nil, nil
	}
	if err != nil {
...
func CreateUpstreamTlsContext(ctx xds_context.Context, metadata *core_xds.DataplaneMetadata, mesh string, upstreamService string, sni string) (*envoy_auth.UpstreamTlsContext, error) {
	...
	commonTlsContext, err := createCommonTlsContext(ctx, metadata, envoy_tls.MeshCaResource, envoy_tls.IdentityCertResource, validationSANMatcher)
	if err != nil {
		return nil, err
	}
...
func createCommonTlsContext(ctx xds_context.Context, metadata *core_xds.DataplaneMetadata, caSecretName, certSecretName string, validationSANMatcher ...*envoy_type_matcher.StringMatcher) (*envoy_auth.CommonTlsContext, error) {
	meshCaSecret, err := sdsSecretConfig(ctx, caSecretName, metadata)
	if err != nil {
		return nil, err
	}
	identitySecret, err := sdsSecretConfig(ctx, certSecretName, metadata)
	if err != nil {
		return nil, err
	}
...
func KumaIDMatcher(tagName, tagValue string) *envoy_type_matcher.StringMatcher {
	return &envoy_type_matcher.StringMatcher{
		MatchPattern: &envoy_type_matcher.StringMatcher_Exact{
			Exact: envoy_tls.KumaID(tagName, tagValue),
		},
	}
}
